...
- Every device takes a name, a room and a kind, set in its panel and used for reading and grouping in the interface. The device list and the rule lists can be sorted into groups by room or by kind, the rule lists going by the device a rule acts on
