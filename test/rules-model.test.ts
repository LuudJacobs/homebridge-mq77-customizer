...
describe('parseRule', () => {
  ...
  it('refuses a rule stored as a timer, which is not a kind any more', () => {
    const timer = parseRule(
      { name: 'Light out', kind: 'timer', triggers: [trigger], waitMs: 30_000, actions: [action] },
      't1',
    );
    // The store drops these on load, so nothing should be offering one.
    expect(timer).toMatchObject({ error: 'Timers are automations with a wait now' });
  });
